...
async def start(_, m: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("Channel", url="https://t.me/feelings_for_you_baby"),
                InlineKeyboardButton("Support", url="https://t.me/Tabahi_chatting_group"),
            ],
            [
                InlineKeyboardButton(
                    "Source code", url="https://graph.org/file/8d930875865ac61996057.mp4"
                )
            ],
        ]
    )
...
async def bin(_, m: Message):
    if len(m.command) < 2:
        ...
        try:
            ...
            caption = f"""
    ╔ Valid :- `{res} ✅`\n╚ Bin :- `{bi}`\n\n╔ Brand :- `{ve}`\n╠ Type :- `{ty}`\n╚ Level :- `{le}`\n\n╔ Bank :- `{ban} ({co})`\n╠ Country :- `{nm} {em}`\n╠ Alpha2 :- `{cod}`\n╚ DialCode :- `{dial}`\n\n**↠ Checked By :-** {mfrom}\n**↠ __Bot By :-** [Devil XD](https://t.me/Assaulted_dark)__
    """
